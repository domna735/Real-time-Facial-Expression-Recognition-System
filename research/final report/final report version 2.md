...
The experiment order followed a risk-managed sequence: KD baseline first to confirm gate stability, then KD+LP, with DKD and other extensions deferred until the above were stable. Results are reported in Sections 4.11–4.13.
...
**Hypotheses for the absence of macro-F1 improvement:**
...
#### 4.4.2 Quantitative NegL screening snapshot (macro-F1 regression evidence)

The following table summarises the macro-F1 outcomes from the Jan 2026 NegL screening runs on HQ-train validation. These numbers motivate the decision not to deploy NegL as a drop-in improvement.

**Table 4.4-2A: NegL screening — macro-F1 on HQ-train validation**

| Configuration | HQ-train macro-F1 | Δ vs KD baseline | Verdict |
| --- | --- | --- | --- |
| KD baseline (5 ep) | 0.7266 | — | baseline |
| KD + NegL | 0.7198 | −0.007 | minor regression |
| KD + NegL + NL(proto) | **0.5204** | **−0.206** | severe collapse |

The KD + NegL + NL(proto) collapse (macro-F1 dropping from 0.7266 to 0.5204) demonstrates that stacking auxiliary losses without per-component ablation can cause catastrophic interference. This reinforces the project's "one knob at a time" methodology: each component must be validated in isolation before composition.
...
#### 4.5.3 Mechanism signals and failure modes
...
**Temperature policy for replay comparisons.** All webcam replay metrics in this section and in Section 4.9 are computed using the same post-hoc temperature-scaling value (optimised on the HQ-train validation set). When comparing checkpoints trained with different distillation stages (CE vs KD vs DKD), each checkpoint's own calibrated temperature is used. No temperature re-tuning is performed between replay sessions, ensuring that differences in smoothed metrics reflect model behaviour rather than calibration artefacts.
...
- **Hypothesised mechanisms:** Several factors can produce a webcam A/B regression even when an offline gate is passed:
...
Additional evidence (partial comparable; different split): we also evaluated on a Kaggle FER2013 folder dataset (msambare). This uses a different train/test split and is **not** a strict match to the “public test” protocol, but serves as a supplementary external validation:
...
**Limitation: FPS/latency benchmarks not yet reported.** A timed benchmark run on the target device (CPU-only and GPU) has not been conducted as of this submission. FPS and latency-distribution numbers are therefore not claimed in this report. The demo pipeline already logs per-frame timestamps, so running the benchmark is a straightforward next step (see Section 9.2). Given that MobileNetV3-Large is a lightweight architecture designed for mobile/edge deployment, achieving ≥ 25 FPS on a modern CPU is expected but remains an empirical claim to be verified. This gap is significant for a project titled "Real-time" FER and is prioritised in the future work plan.
...
1. **Raw and smoothed metrics are always reported side-by-side** (Tables in Sections 4.6 and 4.9) so that readers can assess how much of the deployment score comes from the model vs the filter.
...
- **Training recipe differences (hidden variables)**
  - A systematic checklist comparison (paper_training_recipe_checklist) reveals that published SOTA numbers often embed hidden recipe advantages. For example: FER2013 SOTA methods commonly use 40×40 centre crops with 300-epoch SGD schedules, whereas this project uses 224×224 with AdamW for 30 epochs; the face-regions paper reportedly selects the best checkpoint by test-set performance (inflating reported numbers); AffectNet results in the literature assume 256→224 random-crop with batch-size 256.
  - These recipe differences mean that a direct accuracy comparison with published numbers conflates model capacity with training recipe, and the gap should not be attributed solely to architecture or distillation method.
...
A consolidated comparison of CE vs KD vs DKD performance on four hard gates is provided as a single artifact-backed table (Section 4.4).
...
The implementation details for the Feb 2026 extensions (LP-loss and post-training evaluation hook) are documented in Section 3.9. Results are reported in Sections 4.11–4.13.
