...
This project rebuilds a reproducible real-time facial expression recognition (FER) system and evaluates both offline classification quality and deployment-facing real-time behavior under domain shift. The system targets a canonical 7-class label space (Angry, Disgust, Fear, Happy, Sad, Surprise, Neutral), and emphasizes artifact-grounded provenance: every run produces validated manifests, checkpoints, and metrics JSONs.
...
- Teacher backbones trained with an ArcFace-style protocol (RN18/B3/CNXT), achieving macro-F1 ≈ 0.781–0.791 on a fixed validation split of 18,165 samples (artifacts: `outputs/teachers/*/reliabilitymetrics.json` + `outputs/teachers/*/alignmentreport.json`).
- Official FER2013 split evaluation (from `fer2013.csv`, protocol-aware): best student (DKD_20251229_223722) achieves accuracy **0.614099** (single-crop) and **0.609083** (ten-crop) on PublicTest; and **0.608247** (single-crop) and **0.612148** (ten-crop) on PrivateTest (summary artifact: `outputs/benchmarks/fer2013_official_summary__20260212/fer2013_official_summary.md`).
...
- A deployable student (MobileNetV3-Large) trained via CE → KD → DKD, where KD/DKD significantly improve temperature-scaled calibration (TS ECE) but do not surpass CE macro-F1 in the main HQ-train test evaluation (artifacts: `outputs/students/**/reliabilitymetrics.json`).
- January 2026 extensions:
  - Offline screening of NL/NegL mechanisms for KD/DKD shows mixed or negative macro-F1 impact under tested short-budget configurations (artifacts: `outputs/students/_compare*.md` + per-run `reliabilitymetrics.json`).
  - Deployment-facing domain shift track adds a reproducible webcam scoring protocol (raw vs smoothed, plus flip-rate/jitter), and initial conservative adaptation (head-only / BN-only) fails an offline safety gate on the eval-only manifest (artifacts: `demo/outputs/*/score_results.json` and `outputs/evals/students/*/reliabilitymetrics.json`).

## Presentation-friendly summary (1 page)

**One-sentence summary:** A reproducible real-time FER system is built using a teacher→student pipeline (KD/DKD) and evaluated under both offline benchmarks and deployment-facing domain shift, with strict artifact-backed reporting to avoid ambiguous claims.
...
What we learned (key findings, bounded by stored artifacts):
...
1) Continue fair comparison using **official** data/splits where papers claim “official protocols”; document access constraints (license / request) and plan for additional compute/storage needs (may require applying for or seeking more funding).
2) Even when results look close, treat different papers as different experimental systems: normalize comparisons by author-specific protocol differences (preprocessing/alignment, label mapping, splits, metrics).
3) For remaining gaps, prioritize controlled ablations for likely setting differences (input resolution, crop ratio/TTA, augmentation strength, optimizer/LR schedule, KD/DKD temperatures and weights).
4) Continue domain shift improvement via **safety-gated** Self-Learning + NegL (promote only if eval-only/ExpW gates pass).
...
To make the evaluation reproducible and self-defending, we snapshot the exact local dataset copies used in this workspace using:
...
- We performed an official-split evaluation using a local Kaggle/ICML-format `fer2013.csv` (Usage=Training/PublicTest/PrivateTest). Because it is license-restricted, we do not redistribute it, but we store:
...
Important data note (from each run’s `alignmentreport.json`):
...
Intended objective: improve calibration (ECE/NLL) and reduce overconfident mistakes during KD/DKD by adding a **negative learning** term that discourages probability mass on likely-wrong classes.
...
### 3.6.2 Complementary Learning (NL(proto)): intent and configuration transparency
...
To avoid any ambiguity: the Stage-A validation numbers above are **not** the same evaluation as the hard-gate tests.
...
To answer the practical question “is the model overall good?” without over-claiming comparability to any single paper protocol, we generate one consolidated table across four stress-test manifests:
...
- The consistent fragility remains minority classes (Fear/Disgust), supporting the domain-shift + label-noise framing rather than a simple “model is bad” conclusion.
...
### 4.6.1 Feb 2026 addendum: subjective real-time checkpoint preference (deployment objective mismatch)

Deployment-facing observation (subjective; not an artifact-backed numeric claim):

- In live webcam use, the **CE** student checkpoint (`outputs/students/CE/mobilenetv3_large_100_img224_seed1337_CE_20251223_225031/best.pt`) feels **more stable** (fewer visible label flickers) and has better perceived accuracy than the tested KD+LP and DKD checkpoints.
...
Professor hypothesis (interpreted in deployment terms): “looks like overfitting” under webcam domain shift

- The “overfitting” risk discussed here is **not** classic training-domain overfitting (memorizing training samples). Given the dataset scale and the normal offline metrics behavior, that is not the primary suspicion.
...
Evidence plan (to convert subjective preference into an artifact-backed comparison):
...
- Both head-only and BN-only fine-tuning (as configured) reduce offline macro-F1 on a broader eval-only distribution; these checkpoints should not be promoted beyond experiments.
...
FER2013 example (most common mismatch): many papers report **ten-crop** accuracy, while our current official-split evaluation is **single-crop**. In that case, we report our single-crop result as evidence, but we do not claim strict equivalence.
...
### 6.1.3 Where we “lose” and why that is reasonable
...
- Implement a safe self-learning + NegL ablation in the domain-shift pipeline with strict default-off gating.
...
- A deployment-facing domain shift loop with explicit real-time metrics and an offline regression gate that currently blocks unsafe adapted checkpoints.
...
- Add FER2013 **ten-crop** evaluation (keep single-crop too) and report both blocks side-by-side.
...
This report version (v1.0) formally covers Aug 2025 – Jan 2026. The following notes document Feb-2026 work that updates the codebase and the experimental plan, without claiming new evaluation results unless backed by stored artifacts.
...
1) If the goal is ExpW macro-F1: try smaller `--lp-weight` (e.g., 0.001) and/or `--lp-embed logits`, rerun the same gates.
2) If the goal is deployment stability: use these as candidate base checkpoints for the webcam loop, but do not promote without passing the eval-only gate and re-scoring the *same labeled session*.

### 9.3.5 Feb 2026 addendum (Week 2): Offline benchmark suite + “bad dataset” diagnostics (evidence-backed)

This subsection documents Week-2 Feb-2026 work focused on explaining unexpectedly low benchmark results on (1) eval-only, (2) ExpW, and (3) FER2013 uniform-7.
...
- Bad-datasets investigation report (manifest integrity + per-class F1 comparisons):
...
Key observation (from `reliabilitymetrics_by_source.json`):

- `expw_hq` is substantially weaker than `expw_full` on eval-only macro-F1, and shows especially poor Fear/Disgust F1.
...
- The eval-only “bad result” is not uniform across all sources; it is driven primarily by source composition (notably `expw_hq`) and minority-class fragility.
...
- Disabling CLAHE makes both ExpW and FER2013 worse on macro-F1, so CLAHE is not the main cause of the “bad dataset” results.
...
#### C) Error sampling lists (for manual inspection)

To enable quick, targeted inspection of hard cases (and potential label noise), per-sample `preds.csv` was generated and filtered into short lists:
...
#### D) Interpreting “mixed-source” offline tests (why results can look bad)

This project uses several offline tests that intentionally mix multiple sources/domains (e.g., `classification_manifest_eval_only.csv`, and historically `test_all_sources.csv`). These are valuable, but they are also where scores can look “not good” even when the model is strong on cleaner single-dataset tests.
...
- Note: `fer2013.csv` is license-restricted, so we store paths + derived manifests + metrics artifacts for reproducibility, but do not redistribute the dataset.

### 9.3.6 Feb 2026 addendum (Week 2): Evidence-backed comparison with provided papers (PDFs)

This subsection records a *bounded* comparison against the papers placed in `research/paper compared/`. Because papers often differ in **label space**, **dataset split**, and **metric definition** (accuracy vs macro-F1, balanced vs imbalanced test), this comparison is used to contextualize results rather than claim strict SOTA equivalence.
...
Our canonical numbers (offline suite):

- `outputs/benchmarks/offline_suite__20260208_192604/benchmark_results.csv`

Table 9.3.6-0 (Offline suite export). Dataset: multiple evaluation targets defined by `outputs/benchmarks/offline_suite__20260208_192604/benchmark_index.json`. Split: per target manifest/split. Protocol: single-crop unless otherwise stated by the evaluation run metadata. Evidence: `outputs/benchmarks/offline_suite__20260208_192604/benchmark_results.csv` (rows link back to run artifacts via the index).

Additional comparison artifacts (created Feb-2026):

- One-page “paper vs us” table (explicit comparability flags + links to on-disk metrics artifacts):
  - `research/paper_vs_us__20260208.md`
- Consolidated paper protocol/metric extraction notes (quotable lines + limitations):
  - `research/paper_metrics_extraction__20260208.md`
- FER2013 (Kaggle msambare folder dataset) manifest + eval artifact (not the official split):
  - Manifest: `Training_data/fer2013_folder_manifest.csv`
  - Count summary: `outputs/manifest_counts__fer2013_folder.md` (+ JSON: `outputs/manifest_counts__fer2013_folder.json`)
  - Eval artifact (DKD checkpoint, test split): `outputs/evals/students/DKD_20251229_223722__fer2013_folder__test__20260208/reliabilitymetrics.json`

- FER2013 **official split** (PublicTest/PrivateTest from `fer2013.csv`) conversion + evaluation summary:
  - Summary table (protocol-aware): `outputs/benchmarks/fer2013_official_summary__20260212/fer2013_official_summary.md`
  - Manifests (n=3589 each):
  ...
  - Student eval artifacts (CE/KD/DKD × split × protocol): `outputs/evals/students/fer2013_official__*__*test__20260212__{singlecrop|tencrop}/reliabilitymetrics.json`

#### Evidence index (easy to update)

Purpose: a single place to track “what we have on disk” for fair comparison and for domain-shift tracking.

Update rule:

- Add one new row per evaluation target.
- Always link the **manifest** and at least one **metrics JSON** artifact.
- Mark **Comparable** as `Yes` only when split + protocol match the paper (otherwise `Partial`/`No`).

Table 9.3.6-1 (Evidence index). Purpose: map each evaluation claim to its manifest + checkpoint + metrics artifact. This table is designed to be appended as new evaluations are added.
...
Fair paper comparison (highest priority for supervisor-facing claims):

- Prioritize **official** dataset splits/protocols for comparisons (FER2013 PublicTest/PrivateTest from `fer2013.csv` already done). If other paper targets require official data access (license / request), document the acquisition route and keep the split definition explicit.
- Continue expanding protocol-matched evaluations on official data (and maintain storage of derived manifests + metrics JSONs). This may require applying for or seeking more funding if additional licensed datasets, compute time, or storage are needed.
- Treat differences between different authors’ papers as a first-class variable, even if our numbers look close:
  - Different face detection/alignment/crop policies
  - Different dataset packaging/splits and label remapping rules
  - Different metrics and reporting conventions
- Perform structured “gap analysis” even if we are close but not equal:
  - **Protocol gaps:** crop/TTA, preprocessing (alignment/CLAHE), input resolution, label mapping.
  - **Model gaps:** backbone capacity, attention modules, teacher ensemble vs single model.
  - **Training gaps / settings differences:** batch size, optimizer/LR schedule, augmentation strength, class balancing, loss weights, and high-impact hyperparameters (e.g., learning rate, weight decay, margin/scale in ArcFace-style training; KD/DKD temperatures and weights).
- Keep strict protocol separation in reporting: do not mix folder/uniform-7 stress-test results with official PublicTest/PrivateTest results when comparing to paper numbers.

Domain shift mitigation (deployment objective):

- Continue the safe Self-Learning + NegL track for webcam shift: high-confidence pseudo-label updates, conservative parameter updates, and an offline safety gate (eval-only / ExpW) that must pass before accepting an adapted checkpoint.
- Add or standardize deployment-facing metrics for comparisons (flip-rate/jitter + replay-scored accuracy/macro-F1 on the same labeled session) so “looks stable” vs “scores higher offline” disagreements are measurable.


#### C) AffectNet (macro-F1 vs balanced-subset warning)
...
Interpretation (limitation): our test is explicitly a *balanced* subset/manifest, while the paper discusses original vs skew-normalized evaluation on the AffectNet test set; these are not directly comparable. The correct use here is to show that AffectNet is intrinsically challenging (especially minority classes), and that evaluation protocol matters.

#### Bottom-line answer to “is our model not good?” (bounded by evidence)

- On in-domain datasets like RAF-DB and FERPlus, the system is strong and competitive for a real-time student (high accuracy with reasonable macro-F1).
- The “very bad” results are concentrated in cross-domain / mixture scenarios (ExpW / eval-only / FER2013-uniform), consistent with domain shift + minority-class fragility rather than a universally weak model.
- The fairest next step for paper-style comparison is to **match evaluation protocols** (official splits, reported metrics) and then re-run and report both accuracy *and* macro-F1.

Practical suggestion (what to improve next):

- If the concern is mixed-source performance (e.g., `test_all_sources.csv` style): prioritize **targeted robustness improvements** (Fear/Disgust under domain shift) rather than only adding more of the same data.
- If adding data is feasible: add data that specifically covers the failure modes (low-res, occlusion, non-frontal pose, lighting) and/or higher-quality labels for minority classes, then re-run the same gates to verify improvement.
