...
Report Version: 4 (7-chapter restructured)
...
**Thesis statement:** In real-time FER, checkpoint quality should be judged by a dual-gate deployment protocol rather than offline benchmark gains alone, because offline non-regression by itself does not reliably predict replay-domain improvement.

## Executive Summary

**One-sentence summary:** The project delivers a deployable FER prototype with reproducible training/evaluation artifacts and a promotion policy that blocks unsafe checkpoint updates before live use.

**Project deliverables (implementation-facing):**

1. Multi-source data pipeline with manifest validation and provenance tracking (466,284 validated rows, zero missing paths, zero bad labels).
2. Teacher-to-student training stack (RN18/B3/CNXT teachers; MobileNetV3-Large CE/KD/DKD student) with standardized artifact outputs per run.
3. Real-time demo application with temporal stabilisation controls (EMA, hysteresis, vote window), manual labeling workflow, and replay-compatible logs.
4. Dual-gate promotion workflow (offline non-regression + same-session replay) with explicit pass/fail thresholds and archived A/B evidence.
5. Submission-ready evidence inventory mapping report claims to concrete files/paths for examiner audit.

**Application value and operational outcomes:**

- The system is engineered for edge-style CPU deployment with lightweight student checkpoints and reproducible runtime artifacts.
- Calibration-aware deployment control is practical: KD/DKD improved TS ECE (0.050 to 0.027), supporting safer confidence-threshold logic even when macro-F1 did not exceed CE.
- The dual-gate policy prevented a false promotion: an adaptation candidate passed offline checks but failed same-session replay quality metrics, validating the need for deployment-gate evidence before promotion.
- Benchmark positioning is protocol-aware: RAF-DB result is competitive under this setup, while FER2013 official-split results are used as anchored comparison rather than leaderboard-equivalent claims.
...
3. [Chapter 3 — System Design](#chapter-3--system-design)
   ...
   - 3.2 Data and Teacher–Student Learning Framework
   - 3.3 Real-Time Inference and Stabilisation
   - 3.4 Domain Shift and Dual-Gate Evaluation
   
4. [Chapter 4 — Implementation](#chapter-4--implementation)
   - 4.1 Data Preparation and Validation
   - 4.2 Teacher Training Pipeline
   - 4.3 Student Training Pipeline
   - 4.4 Adaptation Candidate Experiments
   - 4.5 Real-Time Demo System

5. [Chapter 5 — Results & Analysis](#chapter-5--results--analysis)
   - 5.1 Dataset Integrity & Preprocessing Results
   - 5.2 Teacher Performance & Ensemble Robustness
   - 5.3 Student Performance (CE/KD/DKD)
   - 5.4 Calibration Analysis
   - 5.5 Domain Shift & Webcam Results
   - 5.6 Adaptation Experiments & Negative Results
   - 5.7 Benchmark Diagnostics & Paper Comparison
...
Real-time facial expression recognition (FER) aims to classify facial expressions from a live stream while meeting constraints that are not captured by typical offline benchmarks. In practice, a deployable FER system must satisfy latency and throughput targets (here, sub-40ms per frame), maintain temporal stability under detector jitter, produce confidence values suitable for threshold-based logic, and remain robust when webcam-domain conditions diverge from curated training distributions. These constraints jointly define the practical problem setting addressed in this project.
...
Accordingly, this report advances one main claim: promotion decisions in real-time FER must be based on a dual-gate criterion (offline non-regression plus same-session replay quality), not on single-gate offline improvements.
...
This project is organized around four research questions. RQ1 asks whether knowledge distillation (KD/DKD) from a multi-teacher ensemble can improve both classification behavior and calibration quality in a lightweight real-time student. RQ2 examines how domain shift from curated training data to webcam deployment affects per-class behavior, with special attention to vulnerable classes. RQ3 evaluates whether self-learning and negative learning can adapt a deployed model safely without broad-distribution regression. RQ4 asks what evaluation protocol is appropriate when offline metrics and deployment behavior can diverge.

The corresponding objectives are to implement a reproducible teacher-student FER pipeline with artifact-grounded provenance, to evaluate CE/KD/DKD systematically across classification and calibration criteria, to characterize minority-class degradation under domain shift, to validate a safety-gated adaptation loop using offline and replay checks, and to document negative results transparently so the operational limits of the system are evidence-backed rather than anecdotal.
...
This report is organized into seven chapters. Chapter 2 establishes the research context by reviewing FER fundamentals, datasets, architectures, loss functions, real-time constraints, and domain shift. Chapter 3 presents system architecture and design decisions for teacher-student learning, real-time inference, and evaluation protocol design. Chapter 4 documents implementation details across data preparation, training, and evaluation infrastructure. Chapter 5 reports experimental outcomes, including both positive findings and formal negative results. Chapter 6 interprets these outcomes in a broader methodological and practical context, with explicit limitations. Chapter 7 consolidates conclusions, lessons learned, and future directions. The appendix provides artifact-oriented technical detail for auditability and reproduction.
...
The system is architected as a multi-stage pipeline designed to ensure data quality, robust training, and reliable deployment. The design objective is to balance (1) classification performance, (2) real-time responsiveness, and (3) deployment stability under domain shift. Deployment constraints include CPU-friendly latency targets, confidence reliability for threshold logic, and low temporal jitter in webcam inference.

At a high level, the workflow begins with multi-source data cleaning and manifest freezing, proceeds through teacher training and softlabel export, and then distills into a lightweight student for deployment. Components interact through explicit artifacts (manifests, checkpoints, softlabels, evaluation JSON files), so each transition is auditable and reproducible.
...
### 3.2.1 Multi-source Data Pipeline
...
### 3.3.3 Deployment Metrics

To evaluate deployment behaviour, the system reports both raw and smoothed metrics. Core indicators include smoothed vs raw macro-F1 and jitter flips/min, so temporal stability and semantic correctness are assessed together rather than conflated into one number.

## 3.4 Domain Shift and Dual-Gate Evaluation

### 3.4.1 Domain Shift Track
...
### 4.2.1 Training Setup and ArcFace Margin
...
### 4.2.2 Ensemble Selection and Softlabel Export
...
### 4.4.1 Self-Learning and NegL Screening
...
### 4.4.2 Safety-Gated Domain Shift Adaptation
...
### 4.4.3 LP-loss Candidate Integration
...
### 4.5.1 Real-time Demo Implementation and Stabilisation
...
### 4.5.2 Evaluation Scripts and Post-training Infrastructure
...
Dataset integrity checks confirmed 466,284 total rows, with zero missing paths and zero invalid labels. This establishes that all downstream training and evaluation outcomes are grounded in a formally validated data substrate rather than partially broken manifests.
...
For baseline teacher training, the verified full multi-source manifest uses split sizes of train=378,965, val=37,862, and test=49,457. The HQ-train manifest uses train=213,144, val=18,020, and test=27,840. The mixed-domain benchmark (`test_all_sources`) contains 48,928 rows with all seven classes represented, enabling cross-domain stress evaluation without class-absence artifacts.
...
**Table 5.3-1: Student stage comparison (HQ-train val).**
...
**Table 5.5-1: Deployment-aligned metrics (raw vs smoothed).**
...
### 5.5.1 Runtime Resource Metrics (CPU Edge Profile)

To complement classification and stability scores with deployment evidence, we profiled the CE student checkpoint on CPU using a fixed single-face replay crop and repeated forward passes (artifact: `outputs/benchmarks/runtime_profile_20260320.json`). This benchmark isolates model inference and excludes detector/UI overhead, so it should be interpreted as model-runtime capacity rather than full end-to-end camera-loop latency.

**Table 5.5-2: Resource profile for MobileNetV3 student (CPU, repeated inference benchmark).**

| Metric | Value |
| --- | ---: |
| Mean inference time (ms) | 23.01 |
| P50 inference time (ms) | 22.44 |
| P95 inference time (ms) | 30.04 |
| P99 inference time (ms) | 34.97 |
| Equivalent throughput (FPS) | 43.46 |
| CPU utilization (normalized across 32 logical cores) | 66.77% |
| Peak working set memory (MB) | 707.95 |
| Peak private memory (MB) | 1970.66 |
| Student checkpoint size `best.pt` (MB) | 48.50 |
| Deployment ONNX size `student_best.onnx` (MB) | 16.05 |

These runtime/resource figures strengthen the deployment argument in two ways: first, inference-time distribution remains within a sub-40ms tail up to P99 in this isolated benchmark; second, model artifact size is compact enough for practical edge packaging, especially when using ONNX export.

Comparability note: live supports differ by run, so cross-run numbers are treated as behavior evidence, while strict promotion decisions use same-session replay A/B.

Protocol note: Section 5.5 reports live-session scoring views, while Section 5.6 reports promotion-gate replay metrics computed under fixed same-session A/B conditions. Consequently, a run identifier may appear in both sections but with different metric values because the scoring protocol and label alignment differ.
...
This motivates the transition to Section 5.6, where adaptation candidates are evaluated under stricter gate logic and replay comparability controls.

## 5.6 Adaptation Experiments & Negative Results

### 5.6.1 NL/NegL Screening Results
...
**Table 5.6-1: NL/NegL Screening Results (KD-stage baseline vs Auxiliaries).**
...
**Mechanism sanity signals (from history logs):**

| Run | Mechanism | Key setting | Applied fraction trend |
| :--- | :--- | :--- | :--- |
| KD_20251230_004048 | NL(proto) | threshold gate (0.2) | 0.2068 (ep0) -> 0.0089 (ep1) -> 0.00007 (ep4) |
| KD_20260101_091806 | NL(proto) | top-k gate (0.1) | 0.1094 across ep0-ep4 |
| KD_20251228_233720 | NegL | entropy threshold (0.7) | 0.03196 (ep0) -> 0.00583 (ep4) |
| KD_20260101_094542 | NegL | entropy threshold (0.4) | 0.16326 (ep0) -> 0.04052 (ep4) |
...
### 5.6.2 Adaptation Experiments and Negative Results
...
The pre-registered gate thresholds used in this report are aligned with `research/domain_shift/evaluation_plan.md`:
- FAIL if macro-F1 drop > 0.01 on eval-only.
- FAIL if minority-F1 (lowest-3) drop > 0.02 on eval-only.
- WIN on ExpW requires minority-F1 +0.01 with macro-F1 non-regression (drop <= 0.01).

![Dual-Gate Decision Flow](figures/fig12_dual_gate_decision_flow.png)
*Figure 5.6: Dual-gate promotion flow showing offline gate thresholds and replay-gate pass/fail branches for checkpoint promotion.*

**Table 5.6-2: Offline Safety Gate (eval-only test).**
...
Scope note: BN-only webcam replay scoring is not used as positive evidence in this report; promotion-relevant conclusions rely on the explicitly documented same-session A/B artifacts.
...
*Figure 5.7: A/B replay metrics visualization demonstrating the regression of the NR-1 adaptation candidate.*

**Table 5.6-3: Same-session Webcam Replay (Smoothed Predictions)**
...
![Replay A/B Delta Summary](figures/fig13_replay_ab_delta.png)
*Figure 5.8: Same-session replay A/B comparison and deltas (Adapted - Baseline), showing jitter improvement with concurrent predictive-quality regressions.*

The replay deltas are: Accuracy -0.061, Macro-F1 -0.058, Minority-F1 -0.023, and Jitter -0.70 flips/min. Given 4,154 replay frames with fixed labels and fixed stabilizer settings, the accuracy drop corresponds to about 254 additional frame-level errors (1,711 -> 1,965). A normal-approximation 95% confidence interval check gives baseline accuracy about [0.573, 0.603] and adapted accuracy about [0.512, 0.542], with no overlap; the gap magnitude is therefore large relative to sampling noise under this fixed replay protocol.

Against pre-registered promotion tolerances, the replay degradation is also decision-relevant: Macro-F1 drop magnitude (0.058) is about 5.8x the non-regression tolerance (0.01), and Minority-F1 drop magnitude (0.023) exceeds the tolerance (0.02). While the candidate marginally reduced jitter, its predictive capability regressed on all quality metrics that govern promotion. This outcome exposed a profound misalignment: **passing an offline multi-dataset gate does not guarantee or even strongly correlate with deployment improvement** in the highly continuous, heavily autocorrelated webcam domain. This finding necessitated the permanent adoption of the dual-gate evaluation protocol, mandating that candidates must clear both offline validation and deterministic webcam replay.
...
| :--- | :--- | :--- |
| **NR-1** | Adapted candidate passed offline safety but regressed on live webcam replay. | Evidence-backed (Table 5.6-3) |
| **NR-2** | Restricted (Head/BN-only) adaptation failed to preserve base features, failing offline gates. | Evidence-backed (Table 5.6-2) |
...
### 5.6.3 LP-loss Screening Results

LP-loss implementation and short-budget screening on KD/DKD baselines:

**Table 5.6-4: KD vs KD+LP-loss (HQ-train validation).**
...
### 5.7.1 Offline Benchmark Diagnostics

![Cross-Dataset Macro-F1 Degradation](figures/fig6_crossdataset_macro_f1.png)
*Figure 5.9: Degradation of generalisation metrics when evaluating models on out-of-domain sources.*

![Teacher Domain Shift Validation](figures/fig7_teacher_domain_shift.png)
*Figure 5.10: Distributional shift mapped via Teacher confidence outputs across varying datasets.*
...
### 5.7.2 Protocol-aware Paper Comparison
...
*Figure 5.11: Comparative benchmarks against standard FER2013 leaderboards.*

**Table 5.7-1: FER2013 official split evaluation (CE/KD/DKD).**
...
Accordingly, these paper comparisons are anchor-level evidence for positioning, not leaderboard-equivalent claims, because preprocessing policy, split handling, and training recipe details are not fully harmonized across studies.
...
Our investigation into real-time FER has yielded several critical insights. First, the systematic comparison of distillation methods confirms that calibration quality and classification boundary quality are separable objectives in this setting. KD and DKD improve confidence reliability, but this does not automatically improve macro-F1 against hard edge cases. For safety-critical applications where confidence thresholds trigger actions, this calibration gain remains operationally valuable even when classwise discriminative gains are limited.
...
Finally, the failure of our adaptation candidate (NR-1)—which passed offline safety checks but regressed in replay deployment metrics—validates the necessity of our **Dual-Gate Evaluation Protocol**. The quantitative replay evidence in Section 5.6.2 supports that this is not a marginal fluctuation but a decision-scale regression. A system that only optimizes offline metrics risks "silent failure," whereas our dual-gate approach catches these regressions before promotion.
...
Mitigation in this project: we used fixed same-session replay A/B with identical labels and stabilizer settings to maximize internal comparability within the available session set.
...
Mitigation in this project: we maintained artifact-grounded run tracking (manifests, checkpoints, calibration files, and reliability JSONs) and explicitly separated live-view evidence from replay-gate evidence.

**Live Comparability Constraint:** Deployment-facing metrics depend on session composition (class supports and behavior context). Therefore, cross-run live comparisons are descriptive unless they are evaluated on same-session replay A/B with fixed labels and fixed stabilizer settings.
Mitigation in this project: promotion-relevant conclusions were restricted to documented same-session replay A/B artifacts rather than mixed-session live aggregates.
...
This project set out to bridge the gap between academic FER benchmarks and the messy reality of real-time deployment. We successfully engineered a reproducible, end-to-end pipeline that ingests raw, multi-source data and distills it into a lightweight, CPU-capable student model with sub-40 ms P99 inference latency in the isolated model benchmark (excluding detector/UI overhead).

The investigation answers our research questions by establishing a deployment-first evaluation principle. Distillation improves calibration reliability, domain shift remains concentrated in minority classes, and offline pass alone is not a safe promotion signal. The resulting methodological contribution is the **Dual-Gate Evaluation Protocol**, which requires both broad-distribution non-regression and same-session replay quality before checkpoint promotion.
...
While this system functions as a robust prototype, several extensions are both feasible and high-impact. The first is to move from frame-independent inference with post-hoc smoothing toward temporal-aware FER architectures. Real facial expressions evolve continuously, so replacing purely static CNN inference with temporal models (for example LSTM-based heads, lightweight 3D-CNNs, or efficient video-transformer variants) is a direct path to improving stability and minority-class continuity under motion.

The second extension is broader deployment validation. Current evidence is strong for controlled sessions, but a larger hardware and user matrix is needed to map the true latency-accuracy frontier across CPU and GPU tiers. In parallel, optimization work on LP-loss should continue with targeted hyperparameter sweeps (weight, layer injection point, and neighborhood size) to test whether locality preservation can deliver gains beyond short-budget screening.

The third extension is adaptation and fairness governance. Future adaptation policies should use better class-balanced replay buffers and stronger drift controls to prevent majority classes from dominating updates. Finally, a dedicated fairness audit across demographic strata is essential before any operational deployment claim, because aggregate metrics alone cannot rule out disparate failure patterns.
...
Artifact path (exact): `outputs/manifest_validation_all_with_expw.json`
...
Artifact path (exact): `outputs/softlabels/_ens_test_all_sources_rn18_0p4_b3_0p4_cnxt_0p2_logit_clahe_20251223_111523/ensemble_metrics.json`
...
- `outputs/students/DKD/mobilenetv3_large_100_img224_seed1337_DKD_20251229_223722/reliabilitymetrics.json`
...
| :--- | ---: | ---: |
| **Macro-F1** | 0.742 | 0.738 |
| **TS Expected Calibration Error (ECE)** | 0.050 | 0.027 |
| **TS Negative Log Likelihood (NLL)** | 0.778 | 0.765 |
...
### 5. Runtime Resource Profile Artifact (`runtime_profile_20260320.json`)

Artifact path (exact): `outputs/benchmarks/runtime_profile_20260320.json`

CPU benchmark record for the CE student checkpoint using repeated single-face inference (fixed replay crop), including latency distribution, CPU utilization estimate, memory footprint sampling, and model file sizes.

| Metric | Recorded Value |
| :--- | ---: |
| **Mean Inference Time (ms)** | 23.01 |
| **P95 Inference Time (ms)** | 30.04 |
| **Equivalent FPS** | 43.46 |
| **CPU Utilization (normalized)** | 66.77% |
| **Peak Working Set (MB)** | 707.95 |
| **Model Size (best.pt / ONNX, MB)** | 48.50 / 16.05 |
...
| 2) Study/apply knowledge distillation | Met | KD/DKD implemented and evaluated in Sections 4.3, 5.3, and 5.4; full ablation study. |
...
| 5) Explore performance enhancement techniques | Met | NL/NegL screening, domain adaptation, and LP-loss are consolidated in Section 5.6. |
...
### Domain-Shift Adaptation Evidence Sources

Primary process documents used to validate adaptation claims in Chapter 5:
...
- **Privacy by Design:** This aligns with privacy-by-design principles taught in Level 3/4 information security modules by minimizing data transfer and limiting retention. Formal legal compliance (for example, GDPR or local regulation) still depends on deployment context, consent workflow, and institutional governance beyond this prototype.
...
**Application in this Project:**
- **Knowledge Distillation as a Security Shield:** By deploying a *Student Model* (MobileNetV3) rather than the *Teacher Model* (ResNet-18/ConvNeXt), the system reduces direct exposure of original training targets and high-capacity teacher representations. The student never sees raw hard-label supervision in deployment and instead learns compressed teacher distributions. This can reduce practical model inversion and membership-inference risk compared with exposing full teacher artifacts, but it should be treated as risk reduction rather than a formal security guarantee.
...
## A.6 Research Question to Evidence Traceability Map

To improve examiner auditability, this table maps each research question to the primary evidence anchors in the report and artifact set.

| RQ | Claim Tested | Primary In-Report Evidence | Primary Artifact Anchors |
| --- | --- | --- | --- |
| RQ1 (KD/DKD effects) | KD/DKD improve calibration more than macro-F1 | Sections 5.3, 5.4; Table 5.3-1 | Student `reliabilitymetrics.json`, calibration outputs |
| RQ2 (domain shift fragility) | Minority classes (Fear/Disgust) collapse under webcam shift | Sections 5.5, 5.7.1; Table 5.5-1 | `score_results.json`, eval-only per-source summaries |
| RQ3 (safe adaptation) | Adaptation can pass offline and still fail replay | Section 5.6.2; Tables 5.6-2 and 5.6-3 | same-session replay A/B artifacts, gate logs |
| RQ4 (evaluation protocol) | Promotion requires offline + deployment gate pass | Sections 3.4.2, 5.6.2, 6.1 | `research/domain_shift/evaluation_plan.md`, replay evaluation outputs |

---

**End of Final Report Version 4**
