...
**One-sentence summary:** A reproducible lightweight real-time FER pipeline is implemented, shown to run on live webcam input, and evaluated using both offline benchmark metrics and deployment-facing replay metrics, with the main contribution being a dual-gate model-promotion protocol under domain shift.
...
**Novelty in one line:** not a new state-of-the-art FER architecture, but a **deployment-oriented evaluation framework** for lightweight FER that combines offline non-regression, deployment replay, and artifact-backed negative-result reporting.
...
**Identified research gap.** To the best of our knowledge, we did not find prior FER work that jointly evaluates lightweight teacher–student distillation across classification, calibration, and deployment-facing stability while also using a formal dual-gate promotion rule requiring both offline non-regression and replay-based deployment improvement. Most FER papers, including recent state-of-the-art methods, optimise for a single offline benchmark number (typically RAF-DB or AffectNet accuracy) and do not evaluate temporal prediction stability, probability-margin dynamics, or the failure modes that emerge when an adapted model is tested on the same domain it was adapted from. This project addresses these gaps by implementing and empirically evaluating a dual-gate protocol across all three dimensions.
...
Because the CE, KD, and DKD comparisons in this report are based on single-seed runs, small differences between checkpoints should be interpreted as directional rather than statistically definitive. The robust conclusion is therefore not that CE is universally superior, but that in this experimental snapshot KD/DKD improved calibration more consistently than they improved macro-F1.
...
The adaptation experiments are presented in three steps: early fine-tuning failures on the offline gate, a corrected adaptation candidate that passes offline non-regression within rounding, and a final replay-based A/B test showing deployment regression despite that offline pass.
...
Taken together, NR-1 to NR-7 support one central conclusion: **offline non-regression is a necessary screening condition, but not a sufficient deployment-promotion condition.**
...
This section is intended for bounded contextualisation rather than leaderboard-style ranking. The purpose is to show where the present system is competitive, where it is not, and which differences are attributable to protocol mismatch versus genuine model or data limitations.
...
The comparison supports three bounded conclusions. First, on RAF-DB the MobileNetV3-Large student is competitive with older ResNet-18-class FER systems despite a smaller parameter budget. Second, on FER2013 the project remains clearly below strong published numbers, and this gap should be treated as real even after accounting for protocol mismatch. Third, the system's strongest weakness is not in curated in-domain testing but in cross-domain and deployment-facing conditions, especially on minority and confusable classes.
...
| FPS / latency (CPU) | ≥ 25 FPS for real-time feel | Approx. 11.1 FPS from one saved live run; controlled benchmark not yet reported | Section 9.2 |

**Existing measurements.** Live classification behaviour (raw vs smoothed macro-F1/accuracy and jitter flips/min) has been captured from per-session scoring artefacts. Together with the saved per-frame runtime logs, these artifacts provide direct evidence that the system operated continuously over full live webcam sessions rather than only in offline replay.

**Timing evidence remains preliminary rather than benchmark-grade.** A controlled timed benchmark run on the target device (CPU-only and GPU) has not been conducted as of this submission. However, one saved live webcam artifact (`demo/outputs/20260227_130315/per_frame.csv`) contains 8,148 logged frames over 732.66 seconds, implying an observed end-to-end average throughput of approximately 11.1 FPS for that specific machine, detector, and runtime configuration. This is useful evidence that the pipeline ran live in an end-to-end setting, but it should not be over-interpreted as a fully controlled deployment benchmark. Accordingly, the report still avoids making a strong verified claim about sustained real-time throughput in FPS terms; instead, the "real-time" claim refers to successful live webcam operation of the full pipeline, while formal throughput benchmarking is treated as a follow-up validation step.

The demo pipeline already logs per-frame timestamps, and the current implementation can now also record runtime FPS and per-frame latency directly, so the benchmark procedure is straightforward: (1) replay a fixed 60-second video clip through the full pipeline (face detection + classification + smoothing), (2) compute median and 95th-percentile per-frame latency, and (3) report sustained FPS under fixed hardware conditions. Given that MobileNetV3-Large (5.4 M parameters, ~0.22 GFLOPs) was designed for mobile/edge deployment, achieving ≥ 25 FPS on a modern laptop CPU remains a plausible engineering target, but it is still an empirical claim that must be verified under controlled measurement.
...
The claims of this report should be read in three levels. The primary contribution is methodological: a dual-gate evaluation discipline for deployment-facing FER updates. The secondary contribution is engineering: a reproducible end-to-end teacher–student FER pipeline with artifact-backed provenance. The tertiary contribution is empirical: calibration gains from KD/DKD, quantified minority-class failure under domain shift, and a documented negative adaptation result showing that offline non-regression does not guarantee deployment improvement.

**What this project delivers.** The primary deliverable is a working, end-to-end real-time FER system in the practical sense that the full webcam pipeline runs live from face detection through calibrated prediction and temporal stabilisation. Its classifier is a MobileNetV3-Large student (5.4M parameters) that achieves 0.742 macro-F1 on the in-distribution evaluation and 86.3% accuracy on RAF-DB — competitive with ResNet-18-based methods that have twice the parameter count (Table 4.12-D). The student benefits from KD/DKD distillation for substantially improved calibration (TS ECE: 0.027), which is important for any downstream system that uses confidence thresholds. The pipeline is fully reproducible with stored manifests, JSON metrics, and checksummed artifacts at every stage. The demo runs on CPU with temporal stabilisation (EMA, hysteresis, vote window) and per-frame logging for offline analysis, although formal FPS benchmarking remains incomplete.
...
- **Timing evidence is incomplete in the current submission:** although the system is designed for real-time use, end-to-end latency and sustained FPS have not yet been fully benchmarked and therefore are not claimed as validated results.
...
| Mar 2026 | Final report | Report consolidation; structural improvements; timed demo KPI benchmarking planned |
...
This project delivers a reproducible, end-to-end real-time FER pipeline spanning multi-source data cleaning, teacher training, ensemble distillation, and deployment-facing evaluation --- with artifact-backed evidence at every stage. In this report, the real-time claim refers to successful live operation of the full webcam pipeline rather than a fully benchmarked FPS target. The results address the four research questions posed in Section 1.1:
...
In summary, the main contribution of this project is not a new benchmark-leading FER model, but a disciplined way to evaluate whether a lightweight FER system is genuinely improving for deployment. The report shows that calibration gains do not guarantee classification gains, and offline non-regression does not guarantee replay improvement under webcam domain shift. That result is negative in one sense, but academically valuable: it defines a clearer boundary between promising offline adaptation and trustworthy deployment promotion.

### 9.2 Future Work

The next steps are prioritised according to what would most strengthen the present dissertation's central claims rather than what would merely expand the codebase:
