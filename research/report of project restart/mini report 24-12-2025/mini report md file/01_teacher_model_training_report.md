...
For evidence-backed benchmarks, “bad dataset” diagnostics, and professor-facing paper comparison tables created in Feb-2026, see:
...
## Feb 2026 addendum (cross-dataset benchmarking context)

Later work evaluated teachers on a fixed, reproducible multi-dataset suite (RAF-DB, FER2013-uniform, FERPlus, AffectNet-balanced) and two “gates” (eval-only + ExpW).

Canonical suite artifacts:

- `outputs/benchmarks/offline_suite__20260208_192604/benchmark_results.csv`

Important interpretation note:

- These teachers were trained on a filtered mixture dominated by FERPlus/AffectNet/RAF-DB (see `alignmentreport.json`). Lower performance on ExpW / FER2013-uniform is consistent with domain shift, not necessarily “teacher is bad overall”.
