...
if ($Help) {
    Write-Host "ViT-B/16 two-stage trainer (Stage A 224 include FERPlus; Stage B 384 exclude FERPlus)."
    Write-Host "Examples:"
    Write-Host "  powershell -ExecutionPolicy Bypass -File scripts\\run_teachers_overnight_vit_2stage.ps1 -Stage AthenB -ManifestPreset full -CudaDevice 2 -EvalEvery 1"
    Write-Host "  powershell -ExecutionPolicy Bypass -File scripts\\run_teachers_overnight_vit_2stage.ps1 -Stage A -ManifestPreset full -CudaDevice 2"
    return
}
...
$LogDir = Join-Path $RepoRoot (Join-Path 'outputs\teachers' (Join-Path '_overnight_logs_2stage' ("ViT_" + $Stamp)))
