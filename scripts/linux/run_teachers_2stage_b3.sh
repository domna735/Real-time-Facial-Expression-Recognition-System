...
# EfficientNet-B3 two-stage trainer for Linux servers.
# Stage A (224): include FERPlus
# Stage B (384): exclude FERPlus, init from Stage A best.pt (fallback checkpoint_last.pt)
#
# Usage examples:
#   bash scripts/linux/run_teachers_2stage_b3.sh                 # default AthenB on current GPU
#   CUDA_DEVICE=0 bash scripts/linux/run_teachers_2stage_b3.sh   # pin to GPU0
#   STAGE=B CUDA_DEVICE=0 bash scripts/linux/run_teachers_2stage_b3.sh
#   CLEAN=1 CUDA_DEVICE=0 bash scripts/linux/run_teachers_2stage_b3.sh
...
echo ""
echo "B3 two-stage run completed."
