...
$LogDir = Join-Path $RepoRoot (Join-Path 'outputs\teachers' (Join-Path '_overnight_logs_2stage' $Stamp))
...
if ($Models -eq 'RN18') {
    $modelSpecs = $modelSpecs | Where-Object { $_.Tag -eq 'RN18' }
} elseif ($Models -eq 'B3') {
    $modelSpecs = $modelSpecs | Where-Object { $_.Tag -eq 'B3' }
}

Write-Host "Logs: $LogDir"
Write-Host "Stage=$Stage Models=$Models ManifestPreset=$ManifestPreset BatchSize=$BatchSize Epochs=$Epochs NumWorkers=$NumWorkers EvalEvery=$EvalEvery Smoke=$Smoke"
...
function Clear-RunLockIfStale {
    ...
    try {
        ...
        # If we can't parse it, don't risk removing unless forced.
        return
    }
...
function Invoke-TeacherTrain {
    ...
    Write-Host "\n=== [$ModelTag] $ModelName | $StageName | img=$ImageSize ==="
    ...
    # Windows PowerShell may treat native stderr output as an error record when $ErrorActionPreference='Stop'.
    # PyTorch can emit warnings to stderr (e.g., ONNX exporter deprecation) even when the run is healthy.
    # So we temporarily relax error handling and rely on $LASTEXITCODE for correctness.
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Teacher run failed for $ModelName ($StageName) (exit code $LASTEXITCODE). See log: $LogPath"
    }
...
foreach ($m in $modelSpecs) {
    ...
    $baseOut = Join-Path $RepoRoot ("outputs\teachers\" + $tag + "_" + $name + "_seed" + $Seed)
    ...
    if ($Stage -eq 'A' -or $Stage -eq 'AthenB') {
        ...
        Invoke-TeacherTrain -ModelTag $tag -ModelName $name -StageName 'StageA' -ImageSize $StageAImageSize -OutputDir $outA -LogPath $logA -IncludeSources $StageAInclude -ExcludeSources '' -InitFrom ''
    }
    ...
    if ($Stage -eq 'B' -or $Stage -eq 'AthenB') {
        ...
        # If Stage A ran in this session, init from its checkpoint_last.pt. Otherwise, user can run Stage B alone by setting InitFrom manually in the script.
        $init = ''
        $outA = $baseOut + "_stageA_img" + $StageAImageSize
        $ckptA_best = Join-Path $outA 'best.pt'
        $ckptA_last = Join-Path $outA 'checkpoint_last.pt'

        # If Stage B already exists, prefer auto-resume by NOT passing --init-from.
        ...
        Invoke-TeacherTrain -ModelTag $tag -ModelName $name -StageName 'StageB' -ImageSize $StageBImageSize -OutputDir $outB -LogPath $logB -IncludeSources '' -ExcludeSources $StageBExclude -InitFrom $init
    }
}
