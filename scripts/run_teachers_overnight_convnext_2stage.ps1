...
if ($Help) {
    Write-Host "ConvNeXt-Tiny two-stage trainer (Stage A 224 include FERPlus; Stage B 384 exclude FERPlus)."
    Write-Host "Examples:"
    Write-Host "  powershell -ExecutionPolicy Bypass -File scripts\\run_teachers_overnight_convnext_2stage.ps1 -Stage AthenB -ManifestPreset full -CudaDevice 0 -EvalEvery 1"
    Write-Host "  powershell -ExecutionPolicy Bypass -File scripts\\run_teachers_overnight_convnext_2stage.ps1 -Stage B -ManifestPreset full -CudaDevice 0 -UnlockStale"
    Write-Host "  powershell -ExecutionPolicy Bypass -File scripts\\run_teachers_overnight_convnext_2stage.ps1 -Stage AthenB -ManifestPreset full -CudaDevice 0 -OutSuffix my_variant_1"
    return
}
...
$LogTag = if ($OutSuffix -and $OutSuffix.Trim().Length -gt 0) { "CNXT_${OutSuffix}_$Stamp" } else { "CNXT_" + $Stamp }
...
Write-Host "\nConvNeXt two-stage run completed."
